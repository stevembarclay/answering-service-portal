...
import { ChevronRight, Phone, Receipt, TrendingDown, TrendingUp, ExternalLink } from 'lucide-react'
            ...
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              {(() => {
                ...
                return top5.map((type) => (
                  <Link
                    key={type.callType}
                    href={`/answering-service/messages?callType=${encodeURIComponent(type.callType)}`}
                    className="group flex items-center gap-3 px-5 py-2.5 border-b border-border last:border-0 hover:bg-muted/50 transition-colors"
                  >
                    <span className="w-28 shrink-0 text-[13px] text-foreground capitalize group-hover:text-primary transition-colors">
