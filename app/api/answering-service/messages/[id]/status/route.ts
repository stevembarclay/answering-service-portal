...
import { setWorkflowStatus } from '@/lib/services/answering-service/messageStatusService'
...
const PatchStatusSchema = z.object({
  workflowStatusId: z.string().uuid().nullable(),
})
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    await setWorkflowStatus(id, context.businessId, context.userId, parsed.data.workflowStatusId)
    return NextResponse.json({ data: { success: true } })
  } catch (error: unknown) {
    logger.error('PATCH /api/answering-service/messages/[id]/status failed', { error })
    return NextResponse.json(
