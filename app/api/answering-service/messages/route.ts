...
export async function GET(request: Request) {
  try {
    ...
    const encodedCallerNumber = url.searchParams.get('callerNumber')
    const callerNumber = encodedCallerNumber
      ? Buffer.from(encodedCallerNumber, 'base64').toString('utf-8')
      : undefined
