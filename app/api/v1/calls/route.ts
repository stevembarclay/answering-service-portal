...
import { ingestCalls, parseCsvCallRows, type CallIngestResult, type RawCallInput } from '@/lib/services/operator/callIngestService'
import { createClient } from '@/lib/supabase/server'
import { sanitizeErrorMessage } from '@/lib/utils/errorSanitizer'
import { logger } from '@/lib/utils/logger'

const RawCallInputSchema = z.object({
  timestamp: z.string(),
  ...
  telephonyStatus: z.enum(['completed', 'missed', 'voicemail']),
  message: z.string(),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const schemaErrors: CallIngestResult[] = []
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      for (const element of body) {
        const parsed = RawCallInputSchema.safeParse(element)
        if (parsed.success) {
          rows.push(parsed.data)
        } else {
          // SAFETY: element is unknown from request JSON; cast only to extract businessId for error reporting
    ...
    const ingestResults = rows.length > 0 ? await ingestCalls(rows, operatorOrgId, allowedIds, source) : []
    const allResults: CallIngestResult[] = [...schemaErrors, ...ingestResults]
