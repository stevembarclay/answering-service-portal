...
import { ingestRows, parseCsvRows, type IngestResult, type ParsedRow } from '@/lib/services/operator/usageIngestService'
...
const ParsedRowSchema = z.object({
  ...
  callTypeBreakdown: z.record(
    z.object({
      calls: z.number(),
      minutes: z.number(),
      inbound_minutes: z.number().optional(),
      outbound_minutes: z.number().optional(),
...
export async function POST(request: NextRequest) {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    rateLimitKey = context.operatorOrgId
  }
  ...
  try {
    ...
    let rows: ParsedRow[] = []
    const schemaErrors: IngestResult[] = []
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      for (const element of body) {
        const parsed = ParsedRowSchema.safeParse(element)
        if (parsed.success) {
          rows.push(parsed.data)
        } else {
          // SAFETY: element is unknown from request JSON; cast only to extract date/businessId for error reporting
          ...
          schemaErrors.push({
            ...
            date,
            status: 'error',
            issue: `Invalid row structure: ${parsed.error.errors[0]?.message ?? 'unknown error'}`,
          })
        }
      }
    }

    if (rows.length === 0 && schemaErrors.length === 0) {
      return NextResponse.json({ error: { message: 'No rows to process', code: 'BAD_REQUEST' } }, { status: 400 })
    }

    const ingestResults = rows.length > 0
      ? await ingestRows(rows, operatorOrgId, allowedIds, authHeader?.startsWith('Bearer ') ? 'api' : 'csv_upload')
      : []
    const allResults: IngestResult[] = [...schemaErrors, ...ingestResults]
    const errorCount = allResults.filter((r) => r.status === 'error').length

    return NextResponse.json({
      data: {
        processed: allResults.filter((r) => r.status === 'processed').length,
        errors: errorCount,
        results: allResults,
      },
