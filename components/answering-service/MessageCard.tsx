    ...
    <div ref={ref} className="relative" onClick={(e) => e.stopPropagation()}>
      ...
      {open ? (
        ...
        >
          ...
          >
            <span className="h-2 w-2 rounded-full bg-border shrink-0" aria-hidden="true" />
            No status
          </button>
          {statuses.map((s) => (
            <button
  ...
  return (
    <article
      className={cn(
        ...
        cardBorderHover.neutral,
        hoverTransitions.card,
        'p-4 cursor-pointer',
        isSelected && 'border-l-2 border-l-primary bg-muted'
      )}
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 space-y-1">
          <div className="flex items-center gap-2">
            ...
            />
            <span className={`${bodyStyles.caption} font-semibold text-foreground`}>
        ...
        <p className={`${bodyStyles.caption} whitespace-nowrap text-muted-foreground`}>
          {format(new Date(message.timestamp), 'MMM d · h:mma')}
        </p>
      </div>

      <p className={`${bodyStyles.small} mt-3 line-clamp-2 text-muted-foreground`}>{message.message}</p>
