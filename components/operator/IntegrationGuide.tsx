        ...
        <TabsContent value="startel" className="space-y-6 pt-4">
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="flex h-[52px] items-center border-b border-border px-5">
              <span className="text-sm font-semibold text-foreground">Which path is right for you?</span>
            </div>
            <div className="divide-y divide-border">
              <PathRow
                label="Zapier (this guide)"
                badge="Quickstart"
                description="Best for operators with low to moderate call volume — roughly under 30 calls per day per client. No code required. Messages appear in the portal within 1–15 minutes depending on your Zapier plan tier (Professional = ~2 min, lower plans = up to 15 min)."
              />
              <PathRow
                label="CMC RestAPI adapter"
                badge="Developer"
                description="Best for high-volume clients or when you need near-real-time delivery. The built-in StarTel adapter polls the CMC RestAPI on a configurable schedule and has no per-call Zapier task cost. Requires API credentials from StarTel support."
              />
            </div>
          </div>
          <GuideSteps
            title="StarTel via Zapier"
            description="StarTel CMC has a native Zapier app. On each new IntelliForm, Zapier fires a trigger and POSTs a JSON payload to your portal. Zapier polls StarTel on an interval — messages land in the portal within 1–15 minutes depending on your plan tier."
            triggerLabel="StarTel CMC → New IntelliForm"
          />
        </TabsContent>
...
function PathRow(props: { label: string; badge: string; description: string }) {
  return (
