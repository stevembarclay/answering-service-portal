...
**Zapier-specific note:** Zapier's StarTel CMC integration is a *poller*, not a true push webhook — Zapier checks StarTel for new IntelliForm records on an interval (1–15 minutes depending on your Zapier plan tier) and then POSTs them to `/api/v1/calls`. This is transparent to the portal: data arrives via the same HTTP endpoint regardless. The latency tradeoff makes Zapier best suited for operators with low to moderate call volume (roughly under 30 calls/day per client). For higher volume or near-real-time delivery, use the pull-based StarTel adapter below, which polls on a configurable schedule with no per-call cost.
...
**When to use:** High call volume, near-real-time delivery requirements, or when ongoing Zapier task costs become a factor. Also the right choice when you want sub-5-minute latency without paying for a higher Zapier plan tier.
