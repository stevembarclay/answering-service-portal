...
export interface ContactRow {
  ...
  role: string | null
  notes: string | null
  /** StarTel CMC numeric member ID — null until operator configures it */
  startelMemberId: number | null
  /** Amtelco IS numeric contact index — null until operator configures it */
...
export function resolveActiveShift(
  ...
  for (const shift of shifts) {
    ...
    if (matched) {
      const shiftStartsAt = localTimeToUtc(localDateStr, shift.start_time, timezone)
      ...
      const escalationSteps = shift.escalation_steps.map((step, i) => {
        ...
        return {
          ...
          startelMemberId: contact?.startelMemberId ?? null,
          amtelcoContactIndex: contact?.amtelcoContactIndex ?? null,
        }
      })
