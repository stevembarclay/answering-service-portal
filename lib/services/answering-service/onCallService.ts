...
export async function getBusinessTimezone(businessId: string): Promise<string | null> {
  ...
  const { data } = await supabase
    ...
    .eq('id', businessId)
    .maybeSingle()
  // SAFETY: The select above fetches `on_call_timezone`; this column is not in the generated
  // types stub but exists in the live schema. Narrowing the shape manually is safe here.
...
export async function loadSchedulerData(businessId: string): Promise<{
  ...
  const [{ data: shiftRows, error: shiftErr }, { data: contactRows, error: contactErr }] =
    await Promise.all([
      ...
      supabase
        .from('on_call_contacts')
        .select('id, business_id, name, phone, role, notes, display_order, created_at, startel_member_id, amtelco_contact_index')
        .eq('business_id', businessId)
        .order('display_order', { ascending: true })
        .order('created_at', { ascending: true }),
    ])
    ...
    contacts: new Map(
      contacts.map((c) => [
        c.id,
        {
          ...
          role: c.role,
          notes: c.notes,
          // SAFETY: startel_member_id and amtelco_contact_index exist on
          // on_call_contacts (migration 20260327100005) but are not yet in
          // database.types.ts.
          startelMemberId: (c as unknown as Record<string, unknown>).startel_member_id as number | null ?? null,
          amtelcoContactIndex: (c as unknown as Record<string, unknown>).amtelco_contact_index as number | null ?? null,
        },
      ])
    ),
  }
