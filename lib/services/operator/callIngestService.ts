...
export interface RawCallInput {
  ...
  /**
   * Our internal business UUID. Required before calling ingestCalls().
   * Pull-based adapters (StarTel, Amtelco) omit this field — the cron resolves
   * externalClientId → businessId before passing records to ingestCalls().
   */
  businessId?: string
  /**
   * The platform's external client identifier (StarTel "Client ID", Amtelco equivalent).
   * Set by pull-based adapters; the cron resolves this to businessId via the
   * businesses.startel_client_id / amtelco_client_id columns.
   * See: docs/handoffs/2026-03-27-ingest-assumption-audit.md §6 (businessId mapping, option c).
   */
  externalClientId?: string
  /**
   * The platform's stable record identifier (StarTel "Form ID" integer).
   * Used for deduplication.
   * TODO: add external_call_id column to call_logs and enforce a unique constraint
   *   before any live pull-based ingest is enabled.
   */
  ...
  recordingUrl?: string           // publicly accessible URL or Supabase Storage path
}
...
export function validateCallRow(
  ...
  if (!row.businessId) {
    return { valid: false, issue: 'businessId is required.' }
  }
...
export async function ingestCalls(
  ...
  for (const row of rows) {
    ...
    if (!validation.valid) {
      void persistCallIngestError(supabase, {
        operatorOrgId,
    ...
    // TODO: add recording_storage_path column to call_logs to persist recording URLs
    // Migration needed: ALTER TABLE call_logs ADD COLUMN recording_storage_path TEXT;
    // SAFETY: validateCallRow above requires businessId to be present and in allowedBusinessIds.
    const businessId = row.businessId as string
    const { data, error } = await supabase
      ...
      .insert({
        ...
        priority,
        portal_status: 'new',
        has_recording: row.recordingUrl ? true : false,
    ...
    if (error || !data) {
      logger.error('Failed to insert call log', { businessId, error })
      void persistCallIngestError(supabase, {
        operatorOrgId,
        businessId,
        row,
      ...
      results.push({ businessId, status: 'error', issue: 'Database write failed.' })
      continue
    }
    ...
    results.push({ businessId, status: 'inserted', callId })
  }

  return results
}

interface PersistCallIngestErrorInput {
  operatorOrgId: string
  businessId: string
  row: RawCallInput
  issue: string
...
async function persistCallIngestError(
  supabase: ReturnType<typeof createServiceRoleClient>,
  input: PersistCallIngestErrorInput
): Promise<void> {
  const businessId = isUuid(input.businessId) ? input.businessId : null

  const { error } = await supabase.from('call_ingest_errors').insert({
  ...
  if (error) {
    logger.error('Failed to persist call ingest error', {
