...
export async function fireWebhookEvent(
  ...
  if (error) {
    logger.error('Failed to look up webhook subscriptions', { operatorOrgId, topic, error })
    return { queued: 0 }
  }

  const subscriptions = (data ?? []) as Array<{ id: string }>
  for (const subscription of subscriptions) {
    void deliverWebhook(subscription.id, topic, payload, 1).catch((err) => {
      logger.error('Unhandled webhook delivery error', { subscriptionId: subscription.id, err })
    })
  }
